...
class GameBoardCounterIterator {
    ...
    /**
     Actions for each type of cell the iterator will find
    */
     ...
     */
    func checkGameBoardBoundariesAtIndex(indexPath: NSIndexPath) -> Bool! {
        
        return indexPath.row >= 0 &&
            indexPath.section >= 0 &&
            indexPath.row < HSGameConstants.numberOfRows &&
            indexPath.section < HSGameConstants.numberOfSections
    ...
    func iterate(startingIndex: NSIndexPath) {
        ...
        while checkGameBoardBoundariesAtIndex(nextIndex)! && shouldIterate {
